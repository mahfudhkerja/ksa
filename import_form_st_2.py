"""
import_form_st_2.py — versi KEDUA/tambahan dari import_form_st.py.
Import data dari file Excel yang DIUPLOAD (bukan link Google Sheet) lewat
kartu "Form Serah Terima 2" di halaman Input Data Produksi, ke tab
FORM_ST_2 di spreadsheet Monitor Bahan Baku (beda dari FORM_ST_1 yang ada
di spreadsheet utama -- lihat SOURCE_KEY "form_st_2" di config.json,
field target_id/target_sheet).

Header & filter SAMA PERSIS dengan import_form_st.py (TARGET_HEADERS,
...
BEDA dari import_form_st.py:
  ...
  - TIDAK dipanggil dari halaman Data Validasi / /api/validasi/refresh-import.
    ...
    import_val_2.py) tiap kali tombol Refresh di halaman Data Gudang
    BJB/BJL diklik, atau manual: `python import_form_st_2.py`.
"""

from import_engine import run_local_excel_import

SOURCE_KEY = "form_st_2"
TARGET_SHEET_NAME = "FORM_ST_2"
...
# Sama alasannya dengan import_form_st.py: bukan sheet mesin produksi,
# filter junk (KG/JAM/TOTAL/JUMLAH dst) malah salah tangkap baris valid --
# dimatikan total.
JUNK_KEYWORDS = []
